...
import { businessCourses } from "@/lib/courses";
      ...
      <section className="py-20 lg:py-32 bg-[var(--rechera-beige)]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <span className="inline-block px-4 py-1.5 rounded-full bg-[var(--rechera-greige)]/40 text-sm font-medium text-[var(--rechera-text)] mb-6">
              Business
            </span>
            <h1 className="text-[var(--rechera-text)] mb-6">営業/その他</h1>
            <p className="text-lg text-[var(--rechera-text-muted)] leading-relaxed">
              売れる営業のポイント、個別相談の流れなど、
              ビジネスの基盤となるスキルを習得できます。
              どんなビジネスにも応用できる普遍的なスキルを身につけましょう。
            </p>
          </motion.div>
        </div>
      </section>
          ...
          >
            {businessCourses.map((course) => (
              <motion.div key={course.slug} variants={itemVariants}>
                <Link href={`/courses/business/${course.slug}`}>
                  <Card className="h-full bg-white border-none rounded-3xl soft-shadow hover:soft-shadow-hover transition-all duration-300 hover:translate-y-[-4px] cursor-pointer">
                    <CardContent className="p-6">
                      <div className="w-12 h-12 rounded-2xl bg-[var(--rechera-beige)] flex items-center justify-center mb-4">
